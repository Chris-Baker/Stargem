 ...
 * @version 1.0
 */
public class KinematicCharacterBroken extends btRigidBody {
	 ...
	 * @param motionState
	 */
	public KinematicCharacterBroken(btDynamicsWorld dynamicsWorld, btRigidBodyConstructionInfo constructionInfo, short group, short collidesWith, MotionState motionState) {
	 ...
	 * @param worldOrigin
	 */
	public KinematicCharacterBroken(btDynamicsWorld dynamicsWorld, btRigidBodyConstructionInfo constructionInfo, short group, short collidesWith, MotionState motionState, Vector3 worldOrigin) {
...
//		pairArray.getCollisionObjects(out, ghost, new int[1024]);
//		//btBroadphasePairArray.getCollisionObjects(out, btCollisionObject other, int[] tempArray)
...
//						
//			btPersistentManifold manifold = worldDispatcher.getNewManifold(ghost, o);
//			
//			o.
//			
//			System.out.println(manifold.getNumContacts() + " Collision " + o);
//			
...
//				if (dist < 0.0f) {
//					// if one of the objects is terrain then we need to only recover directly up from the origin
//					if((o.getContactCallbackFlag() & ContactCallbackFlags.TERRAIN) > 0 || (ghost.getContactCallbackFlag() & ContactCallbackFlags.TERRAIN) > 0) {				
//						motionState.transform.getTranslation(normalWorldOnB);
//						//normalWorldOnB.add(pt.getNormalWorldOnB().getX(), pt.getNormalWorldOnB().getY(), pt.getNormalWorldOnB().getZ());
//						normalWorldOnB.nor();					
//					}
